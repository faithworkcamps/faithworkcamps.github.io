...
document.write('<script type="text/javascript" src="http://64.73.54.11/fflnet'+year+'/mfl_player_database.js"></script>');
...
if (currentURL.indexOf("http://football")!=-1||currentURL.indexOf("http://6")!=-1) var habBaseURL = baseURLDynamic; else var habBaseURL = baseURLStatic;
...
function formatName(name) {
   var tempname = name;
   var tempname = tempname.replace(/'/g,"’");
...
function PlayerOffense (gp, comp, passes, p_yards, p_tds, p_ints, fl, runs, ru_yds, ru_tds, rec, re_yds, re_tds, tds, two_pt, fg_att, fg, pat_att, pat) {
   this.gp      = gp;
   this.comp    = comp;
   this.passes  = passes;
