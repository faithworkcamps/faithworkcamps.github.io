...
if(!habman_common) {
	...
	document.write('<script type="text/javascript" src="http://www.habman.com/mfl/apps/js/habPlayerKey.js"></script>');
		...
		if(!habGlobalLeagueRan) {
			...
			for (var i=0; i < franchises.length; i++) {
				...
				if(franchises[i].getAttribute("logo") == null) 
					habGlobalLogos[fid] = "http://www.habman.com/mfl/images/notfound.gif"; 
				else 
					habGlobalLogos[fid] = franchises[i].getAttribute("logo");
				if(franchises[i].getAttribute("icon") == null) 
					habGlobalIcons[fid] = "http://www.habman.com/mfl/images/notfound.gif"; 
				else 
	...
	function PlayerDefense (gp, tkl_d, tkl_st, ass_d, ass_st, sack, ff, fr, int, td_d, td_st, tds, safety, null_13, null_14, null_15, null_16, null_17, null_18, null_19) {
		this.gp      = gp;
		this.tkl_d   = tkl_d;
		this.tkl_st  = tkl_st;
		this.ass_d   = ass_d;
	...
	function PlayerTeamDefense (gp, pts_d, pts_tot, p_yds, ru_yds, yds, f_tds, int_tds, tds, safety, fr, int, sack, null_13, null_14, null_15, null_16, null_17, null_18, null_19) {
		this.gp      = gp;
		this.pts_d   = pts_d;
	...
	function PlayerSpecialTeam (gp, tds_f, safety, tds_a, kr, kr_yds, kra, kra_yds, pr, pr_yds, pra, pra_yds, blk_p, blk_pa, null_14, null_15, null_16, null_17, null_18, null_19) {
		this.gp      = gp;
		this.tds_f   = tds_f;
		this.safety  = safety;
		this.tds_a   = tds_a;
			...
			for (team in franchiseDatabase) {
				if (team.search(/fid_/) == 0) { //fid_ is found therefore valid franchise ID tag
					...
					if(franchiseDatabase[fid].logo == null) 
						habGlobalLogos[habTeamID(i)] = "http://www.habman.com/mfl/images/notfound.gif"; 
					else 
						habGlobalLogos[habTeamID(i)] = franchiseDatabase[fid].logo;
					if(franchiseDatabase[fid].icon == null) 
						habGlobalIcons[habTeamID(i)] = "http://www.habman.com/mfl/images/notfound.gif"; 
					else 
