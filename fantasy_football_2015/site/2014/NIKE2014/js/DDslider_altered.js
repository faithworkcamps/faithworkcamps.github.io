/*
 * DDSlider v1.4 - http://codecanyon.net/item/ddslider-10-transitions-inline-content-support/104797
 * 
    ...
    $j.fn.extend({ 
        DDSlider: function() { 
			...
			var iNum = 1;
				...
				this.children('li').each(function() {
					...
					if(attr.selector === undefined) {
						...
						if(iNum == 1) {
							$j(attr.selector).append('<li class="current sel_'+iNum+'"></li>');
						} else {
							$j(attr.selector).append('<li class="sel_'+iNum+'"></li>');
						}
			...
			if(attr.prevSlide === undefined) {  } else {
				...
				$j(attr.prevSlide).click(function(){
			...
			if(attr.nextSlide === undefined) {  } else {
				...
				$j(attr.nextSlide).click(function(){
			...
			$j(attr.selector).children('li').click(function() { var itemId = $j(this).attr('class').split(' ');
			...
			$j(this).hover(function() { // Whenever an item is hovered
		...
		nextSlide: function(attr) {
			...
			//finds selectors
			var currentSel = $j(attr.selector).children('li.current');
			var nextSel = $j(attr.selector).children('li.current').next();
			
			//if the is no next one, choose the first
			if(nextItem.length > 0) {
				//Do nothing. The  $jnext element exists
			} else {
				nextItem = this.children('li:first');
				nextSel = $j(attr.selector).children('li:first');
			}
		...
		prevSlide: function(attr) {
			...
			//finds selectors
			var currentSel = $j(attr.selector).children('li.current');
			var prevSel = $j(attr.selector).children('li.current').prev();
			...
			if(prevItem.length > 0) {
				...
				prevSel = $j(attr.selector).children('li:last');
			}
		...
		callSlide: function(slideID, attr) {
			...
			var currentSel = $j(attr.selector).children('li.current');
			var nextSel = $j(attr.selector).children('li.sel_'+slideID);
		...
		nextTransition: function(attr, transNext, transCur, transSelNext, transSelCur) {
			...
			if(nextTransition == 'random') {
				...
				var nextTransShuffle = $j.shuffle(arr_trans);
		...
		DDBarTop: function(attr, barNext, barCur, nextSel, curSel) {
			...
			barNext.animate({ opacity: 0 }, totalDelay).animate({opacity: 0}, attr.duration, function() {
				...
				$j(this).addClass('current').css({ opacity: 1 });
		...
		DDBarFadeRandom: function(attr, barNext, barCur, nextSel, curSel) {
			...
			var bars_array_shuffle = $j.shuffle(bars_array);
		...
		DDSquare: function(attr, squareNext, squareCur, nextSel, curSel) {
			...
			while(i_row_numbers <= attr.rows) {
				...
				while(i_column_numbers <= attr.columns) {
					...
					$j(animated_class).animate({ width: row_width }, delay).animate({ opacity: 1 }, {duration: attr.duration, easing: attr.ease});
		...
		DDSquareRandom: function(attr, squareNext, squareCur, nextSel, curSel) {
			...
			var squareArrShuffle = $j.shuffle(square_arr);
		...
		DDRowInterlaced: function(attr, squareNext, squareCur, nextSel, curSel) {
			...
			while(i_row_numbers <= attr.rows) {
				...
				$j(animated_class).animate({ opacity: 0 }, delay).animate({ left: 0, opacity: 1 }, {duration: attr.duration, easing: attr.ease});
