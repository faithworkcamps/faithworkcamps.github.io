...
//If you want to host your own files copy these files and re-link to your hosting site
	...
	var styleSheet12 = "http://nitrografixx.com/cbsscoreboard/css/ptd2014scoreboard.css";
	...
	var styleSheet16 = "http://nitrografixx.com/cbsscoreboard/css/dmd_scoreboard.css";
	var styleSheet17 = "http://nitrografixx.com/cbsscoreboard/css/hag_scoreboard.css";
	...
	var playerGameStatsPHP = "http://nitrografixx.com/habman_js/2015SkinScoreboardPlayerGameStats.php";
	var playerNewsPHP = "http://nitrografixx.com/habman_js/2015SkinScoreboardPlayerNews.php";
	var playerLoadingPHP = "http://nitrografixx.com/habman_js/2015SkinScoreboardPlayerLoading.php";
...
if(useStyleSheet==undefined) var useStyleSheet = -1;
if(scoreboardOnTop==undefined) var scoreboardOnTop=true;
...
if(includeNewsBreakers==undefined) var includeNewsBreakers=false;
...
if(includeHabChat==undefined) var includeHabChat=0;
if(includeBorderlessWindow==undefined) var includeBorderlessWindow=false;
if(includeCSSInPopups==undefined) var includeCSSInPopups=0;
if(includeNFLVideo==undefined) var includeNFLVideo=false;
...
if(scoreboardImage==undefined) var scoreboardImage = 3;
...
if(location.href.indexOf("MODULE=MESSAGE")>0||location.href.indexOf("/message")>0||location.href.indexOf("SEQNO=")>0) { //GOOD TO GO
	...
	if(location.href.indexOf("m=0")>0||location.href.indexOf("M=0")>0) scoreboardLayout=0;
	if(location.href.indexOf("m=1")>0||location.href.indexOf("M=1")>0) scoreboardLayout=1;
	if(location.href.indexOf("m=2")>0||location.href.indexOf("M=2")>0) scoreboardLayout=2;
	if(location.href.indexOf("m=3")>0||location.href.indexOf("M=3")>0) scoreboardLayout=3;
	if(location.href.indexOf("m=4")>0||location.href.indexOf("M=4")>0) scoreboardLayout=4;
	...
	if(useStyleSheet==16) { document.write("<link rel='stylesheet' href='"+styleSheet16+"' type='text/css' />"); myStyleSheet = styleSheet16; }
	if(useStyleSheet==17) { document.write("<link rel='stylesheet' href='"+styleSheet17+"' type='text/css' />"); myStyleSheet = styleSheet17; }
	...
	//====================================================================================
		...
		function createWeeklyNavigation() {
			...
			for (var i=parseInt(cbsLeagueInfo[3]);i<=parseInt(cbsLeagueInfo[4]); i++) {
				...
				if(i!=parseInt(cbsLeagueInfo[4])) htmlCode = htmlCode + "<span class='navdash'>|</span>";
			}
		...
		function createCBSMatchupTable(matchupInfo,which) {
			...
			if(which<matchupInfo.length) {
				...
				htmlCode = htmlCode + "        <tr class='matchupLolite' title='View Game Score' id='matchup_" + which + "' onclick='setCBSMatchup(" + which + ")'></tr>\n";
				htmlCode = htmlCode + "        <tr><td style='text-align:left; padding-left:3px;'"+roadTitle+">" + roadName + "</td><td style='text-align:right; padding-right:3px;' id='fid_" + matchupInfo[which]['road'][0] + doubleHeader[matchupInfo[which]['road'][0]]+"'"+roadTitle+"></tr>\n";
		...
		function createMainScoreboardTable() {
			...
			scoreboardHTML = scoreboardHTML + "    </tr>\n";
			scoreboardHTML = scoreboardHTML + "    <tr id='cbsmessage'>\n";
			scoreboardHTML = scoreboardHTML + "     <td id='cbsScoreboardMessage'><span id='cbsScoreboardMessageResetTimer' onclick='setCBSMatchup("+currentGameHilighted+")'>&nbsp;Click To Retart&nbsp;</span></td>"; 
			scoreboardHTML = scoreboardHTML + "    </tr>\n";	
			scoreboardHTML = scoreboardHTML + "    <tr>\n";
			...
			myTable = "<table class='cbsTeamLineup'>\n";
				for(var i=0;i<3;i++) {
						...
						while (playerList.indexOf(",")>0)	{
							...
							if(i!=2||(i==2&&playerID!=tiebreakingPlayer)) { // tiebreaking player also in bench list
								...
								try {
									...
									tableRows[rowCount] = new Array(getPositionSortNumber(cbsPlayerInfo[playerID][2]),"<tr" + rowStyle + "><td class='cbsPlayerName' style='border-right:0px; white-space:nowrap;'>&nbsp;<a href='" + baseURLDynamic + "/" + year + "/player?L=" + league_id + "&P=" + playerID + "' target='mflpage'>" + cbsPlayerInfo[playerID][0] + "&nbsp;" + cbsPlayerInfo[playerID][2] + "</a>" + injury + newsIcon + "</td><td class='cbsPlayerGame' style='border-right:0px; white-space:nowrap; vertical-align:middle;'>&nbsp;" + nflTeamDisplay + nflGameDetail + "</td>"+SOS+"<td class='cbsGameClock' style='border-left:0px; white-space:nowrap;'>&nbsp;" + nflGameStatus + "</td>"+projections+"<td class='cbsPlayerPoints' title='View Player Stats' onclick='openStatsWindow(event,\""+playerID+"\",\""+which+"\",\""+cbsCurrentWeek+"\");'>" + pointsEarned + "&nbsp;</td></tr>\n");
									rowCount++;
								} catch(er) {
					...
					if (tableRows.length>0) {
						...
						if(includeProjections) {
							if(includeSOS) {
								var headerRow = "<tr class='lineupHeaderRow'><th class='lineupHeaderPlayer'>"+headerTitle+"</th><th class='lineupHeaderGames'>Games</th><th class='sos'>S.O.S</th><th class='gamestime'>Time</th><th class='lineupHeaderProj'>Proj</th><th class='lineupHeaderActual'>Actual</th></tr>\n";
							} else {
						...
						if (doTotals) {
							if(includeProjections) {
								if(includeSOS) {
									...
									myTable = myTable + "<tr class='pointTotalRow'><th colspan='3' class='pointTotalTitle'>&nbsp;"+headerTitle+" Totals</th><th class='pointTotalProj'>"+tableProjTotals.toFixed(precision)+"&nbsp;</th><th class='pointTotalActual'>"+tableTotals.toFixed(precision)+"&nbsp;</th></tr>\n";
								}
							} else {
								if(includeSOS) {
									...
									myTable = myTable + "<tr class='pointTotalRow'><th colspan='3' class='pointTotalTitle'>&nbsp;"+headerTitle+" Total</th><th class='pointTotalActual'>"+tableTotals.toFixed(precision)+"&nbsp;</th></tr>\n";
								}
							}
						}
					}
	...
	//====================================================================================
		function setCBSMatchup(which) {
			...
			for(var x=0;x<cbsLiveScoringMatchups.length;x++) { //reset all my links game hilights to original colour
				document.getElementById("matchup_" + x).parentNode.setAttribute("class", "matchupLolite");
			}
			document.getElementById("matchup_" + which).parentNode.setAttribute("class", "matchupHilite");
		...
		function startTheTimer() {
			...
			if(maxLoops!=0&&myLoopCount>maxLoops) { //stop the refreshes to conserve XML executions
				document.getElementById("cbsScoreboardMessage").innerHTML="<span id='cbsScoreboardMessageResetTimer' onclick='setCBSMatchup("+currentGameHilighted+")'>Click To Restart</span>";
			} else {
...
//var tpid = '8301'; var tscore = 9.5; addToPlayerUpdate(tpid, parseFloat(parseFloat(playerUpdates[1][tpid])).toFixed(precision), parseFloat(tscore-parseFloat(playerUpdates[1][tpid])).toFixed(precision),parseFloat(tscore).toFixed(precision));
	...
	//==FIRST THING I WANT IS ONE TABLE TO HOLD ALL OF MY OTHER NESTED TABLES; CLASS NAME WILL BE cbsOuterTable	
		...
		if(liveScoringEmpty) document.getElementById("cbsScoreboardMessage").innerHTML = "<span id='cbsScoreboardMessageResetTimer' onclick='reloadThisPage();' style='background-color:red; color:white;'>&nbsp;Click To Refresh&nbsp;</span>";
