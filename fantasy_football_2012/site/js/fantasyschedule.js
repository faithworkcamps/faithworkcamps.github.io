...
if(fantasyScheduleMaximumWeek==undefined) var fantasyScheduleMaximumWeek = 17;
...
function doWeeklyResultsFantasySchedule (matchups,weekNum) {
	...
	var htmlCode = "<table align='center' cellspacing='0' class='homepagemodule report' id='fantasyschedule'><caption><span>Fantasy Schedule</span></caption>\n";
	htmlCode += " <tbody>\n";
	htmlCode += "   <tr class='oddtablerow'><td colspan='8' class='weekNav'><center>" + arrowLeft + " Wk#" +weekNum + " " + arrowRight + "</center></td></tr>\n";
	...
	for (var i = 0; i < matchups.length; i++) {
		...
		if(roadID!="BYE"&&homeID!="BYE") {
			...
			if(matchups[i]['road'][0]="AVG") var altRoadName = "AVG"; else 	var altRoadName = franchiseDatabase['fid_' + matchups[i]['road'][0]].name;
			if(matchups[i]['home'][0]="AVG") var altHomeName = "AVG"; else 	var altHomeName = franchiseDatabase['fid_' + matchups[i]['home'][0]].name;
			if (altRoadName!=undefined) altRoadName = altRoadName.replace(/'/g,"&rsquo;");
			if (altHomeName!=undefined) altHomeName = altHomeName.replace(/'/g,"&rsquo;");
			...
			if(weekNum<=completedWeek) {
				...
				if(parseFloat(roadScore,10)>parseFloat(homeScore,10)) {
					roadWinIcon = "<img src='http://www.nitrografixx.com/habman_js/roadwin.gif'  style='vertical-align: middle; height: 15px; width: 9px;' />";
					homeWinIcon = "<img src='http://www.nitrografixx.com/habman_js/blankwin.gif' style='vertical-align: middle; height: 15px; width: 9px;' />";
				}
				if(parseFloat(roadScore,10)<parseFloat(homeScore,10)) {
					roadWinIcon = "<img src='http://www.nitrografixx.com/habman_js/blankwin.gif' style='vertical-align: middle; height: 15px; width: 9px;' />";
					homeWinIcon = "<img src='http://www.nitrografixx.com/habman_js/roadwin.gif'  style='vertical-align: middle; height: 15px; width: 9px;' />";
				}
			} else {
...
function doLiveFantasySchedule(matchups,weekNum) {
	...
	var htmlCode = "<table cellspacing='0' align='center' class='homepagemodule report' id='fantasyschedule'><caption><span>Fantasy Schedule</span></caption>\n";
	htmlCode += " <tbody>\n";
	htmlCode += "   <tr class='oddtablerow'><td colspan='6' class='weekNav'><center>" + arrowLeft + " Wk#" +weekNum + " " + arrowRight + "</center></td></tr>\n";
	...
	for (var i = 0; i < matchups.length; i++) {
		...
		if (roadID!="BYE"&&homeID!="BYE") {
			var remainSecRoad = matchups[i]['road'][2];
			var remainSecHome = matchups[i]['home'][2];
			if(weekNum>liveScoringWeek) { // get schedule only for leagues that have starting lineups hidden
				var roadScore = "&nbsp;";
				var homeScore = "&nbsp;";
			} else {
				var roadScore = parseFloat(matchups[i]['road'][1],10).toFixed(fantasyScheduleDecimals);
			...
			altRoadName = altRoadName.replace(/'/g,"&rsquo;");
			altHomeName = altHomeName.replace(/'/g,"&rsquo;");
...
function getFantasySchedule(thisWeek) {
	...
	if(fantasyScheduleMaxLoops<31) {
		if(thisWeek==0) {
			...
			} else thisFantasyWeek  = liveScoringWeek;
		} else thisFantasyWeek  = thisWeek;  
		if(thisFantasyWeek>17) thisFantasyWeek = 17;
		if(thisFantasyWeek>fantasyScheduleMaximumWeek) thisFantasyWeek = fantasyScheduleMaximumWeek;
		...
		if(liveMode) {
			...
			if(thisWeek>liveScoringWeek) { // use live scoring to get schedule in case starters are hidden
				fantasyScheduleLiveScoring   = makeSyncHttpRequest(habBaseURL+"/"+year+"/export?TYPE=liveScoring&L="+league_id+"&W="+thisFantasyWeek+"&rand=" + Math.random() , 'parseHabLiveScoringXML' , 'liveScoring',true );
				doLiveFantasySchedule(fantasyScheduleLiveScoring,thisFantasyWeek);
			} else {
